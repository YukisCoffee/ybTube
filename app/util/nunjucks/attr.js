 ...
 * value.
 */
function NunjucksAttributesHelper(attributes) {
    var output = "";
...
exports.default = NunjucksAttributesHelper;
const TRANSFORM_TOKENS = ["@", "*"];
/**
 * Allow the use of some characters to apply
 * specific attributes.
 */
function transformName(orig) {
    ...
    for (var originalToken of TRANSFORM_TOKENS) {
        ...
        var tokenRegexp = new RegExp(`(?<!\\\\)${t}$|(?<!.)${t}`, "m");
    ...
    nyuu = nyuu.replace("\\", "").replace(/[^a-zA-Z0-9]/g, c => `&#${c.charCodeAt(0)};`);
